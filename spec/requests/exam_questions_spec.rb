...
RSpec.describe "ExamQuestions" do
  ...
  let!(:word1) { create(:word, english_word: "apple", user: login_user) }
  let!(:exam_question1) { create(:exam_question, exam: exam1, word: word1, is_correct: nil) }
  ...
  describe "POST /update" do
    context "単語テストでの回答が正解の場合" do
      it "is_correctカラムをtrueで上書きすること" do
        patch exam_question_path(exam_question1.id), params: { exam_question: { is_correct: "apple" } }
        expect(exam_question1.is_correct).to be true
      end
    end

    context "単語テストでの回答が不正解の場合" do
      it "is_correctカラムをfalseで上書きすること" do
        patch exam_question_path(exam_question1.id), params: { exam_question: { is_correct: "wrong_answer" } }
        expect(exam_question1.is_correct).to be false
      end
    end
    ...
    context "単語テストが10問目以下の場合" do
      let!(:less_than_ten_exam_questions) { create_list(:exam_question, 8) }
